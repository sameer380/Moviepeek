  ...
  return (
    <>
      <Backdrop opened={opened}>
        <div ref={ref} className={clsx(MODAL_CLASS_NAME, className)} {...rest}>
          {title && (
            <div className={MODAL_HEADER_CLASS_NAME}>
              <h2>{title}</h2>
            </div>
          )}
          {body && <div className={MODAL_BODY_CLASS_NAME}>{body}</div>}
          {footer && <div className={MODAL_FOOTER_CLASS_NAME}>{footer}</div>}
        </div>
      </Backdrop>
      <style jsx>{`
        .${MODAL_CLASS_NAME} {
