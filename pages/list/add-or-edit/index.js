...
const AddOrEdit = ({ accountId, accessToken }) => {
  ...
  const submitCallback = async () => {
    try {
      ...
      const body = {
        ...
        // TODO: vulnerable
        public:
          inputs[INPUT_NAMES.PUBLIC_OR_PRIVATE_LIST] ===
          YES_OR_NO_OPTIONS[0].value,
        iso_639_1: 'en',
      };
      const config = {
        headers: {
  ...
  const { inputs, inputChangeHandler, updateInputsHandler, onSubmitHandler } =
    useForm({
      submitCallback,
      initialInputs: {
  ...
  if (editStatus === STATUSES.REJECTED) {
    ...
    return (
      <NotFound
        title="Sorry!"
        subtitle={
          editError?.message ?? 'We can\'t find the page you\'re looking for...'
        }
      />
    );
  }
