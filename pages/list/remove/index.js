...
const Remove = ({ accountId, accessToken }) => {
  ...
  const removeListHandler = async () => {
    try {
      ...
      await tmdbAPI.delete(`/${TMDB_API_NEW_VERSION}/list/${listId}`, {
        headers,
      });
  ...
  if (status === STATUSES.RESOLVED) {
    return (
      <>
        ...
        <PageWrapper>
          <PaddingWrapper>
            <Header title="Delete List" subtitle="Edit" />
            ...
            <Modal
              ...
              title="Are you sure?"
              body={
                <p>By clicking yes, this list will be permanently deleted.</p>
              }
              footer={
                <ActionsWrapper>
                  <TextButton onClick={closeRemoveModalHandler}>No</TextButton>
                  <TextButton onClick={removeListHandler}>Yes</TextButton>
                </ActionsWrapper>
              }
            />
            <FormControl>
              <Label htmlFor="delete-list-button">
                Click the button below if you are sure you want to delete this
                list.
              </Label>
              <Button
