...
const AddOrRemoveItems = ({ accountId, accessToken }) => {
  ...
  const addItemHandler = async (newMediaId) => {
    try {
      ...
      await tmdbAPI.post(
        `/${TMDB_API_NEW_VERSION}/list/${listId}/items`,
        body,
        config,
      );
      const expectedPage = Math.ceil(
        (movies.total_results + 1) / TMDB_PAGE_LIMIT,
      );
  ...
  const removeItemHandler = (mediaId) => async () => {
    try {
      ...
      await tmdbAPI.delete(`/${TMDB_API_NEW_VERSION}/list/${listId}/items`, {
        headers,
        data,
      });
      ...
      setMovies((prevState) => {
        const newResults = prevState.results.filter(
          (result) => result.id !== mediaId,
        );
        const nextState = {
          ...prevState,
          results: newResults,
  ...
  if (status === STATUSES.RESOLVED) {
    return (
      <>
        ...
        <PageWrapper>
          <PaddingWrapper>
            <Header title={movies.name} subtitle="Edit" />
            ...
            {movies.results.length === 0 && page === 1 ? (
              <Annotation>There are no items added to this list.</Annotation>
            ) : (
              <List
                style={{
                  marginTop: 24,
            ...
            <Annotation
              style={{
                visibility:
                  addItemStatus === STATUSES.PENDING ? 'visible' : 'hidden',
                margin: '8px 0',
              }}
            >
